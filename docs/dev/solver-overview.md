...
- `objective`: 主优化目标，当前公开使用的是 `min_buildings`（占地加权）、`min_power`、`min_external_input`
...
- `powerSummary`: 总工作功耗与按工作/待机负载加权估算的功耗
...
- 计算连续建筑数与占地加权建筑成本
- 将生产功耗作为虚拟需求接入物品平衡
- 按连续负载计算工作功耗；展示层的放置功耗会对最后一个未满载建筑按工作/待机功耗插值
- 将发电设施作为数据驱动配方接入；发电规则可选择用增产模式提高功率或用加速模式同时提高功率与燃料消耗
- 生成用户可审计的物品平衡与功耗汇总
...
有一个还保留在核心里的实验目标 `min_complexity`，但 Web 入口目前不暴露它；当前对外仍然以 `最少占地 / 最低功耗 / 最少外部输入` 这三个目标为主。
